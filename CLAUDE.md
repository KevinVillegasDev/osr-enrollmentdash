...
### Color Palette (Lifted Dark Theme, March 2026)
...
- **Analytics-only password** ("adminaccess"): additional gate on analytics.html for Kevin/leadership only.
- Full-screen dark overlay (#0D1321) with password prompt. On correct entry, stores hash in localStorage and removes overlay.
- Detection: checks URL pathname for `/analytics` (with or without `.html` for Netlify clean URLs).
...
6. Monthly Dashboards (cards per month, auto-collapses after 3 most recent)
...
- Projected = MTD Actual × (total biz days ÷ elapsed biz days) ÷ Budget × 100
- Variance = Projected % − 100%
...
- Day filter and type filter (Existing/Prospect)
- Existing/Prospect split bar per rep
- Daily bar chart per rep
- Expandable stop list with full comments
- Deduplication logic (multiple entries per stop → keeps longest comment)
- Timestamps with gap indicators and adaptive bars
...
3. Pulls 6 Salesforce reports via Analytics REST API (v62.0)
...
The Analytics REST API returns reports in different formats depending on report type:

- **SUMMARY** format (Reports 1, 2): Stores raw Salesforce IDs in main keys and display labels in `_label_` prefixed keys. The normalization step converts these to display values.
- **MATRIX** format (Reports 3, 4): Date-prefixed aggregate columns like `"3/1/2026_Sum of Funded Dollars"`. The `_normalize_matrix_to_monthly()` helper splits these into per-month flat rows.
- **TABULAR** format (Reports 5, 6): Straightforward key-value rows. Report 6 has currency fields as `{amount, currency}` dicts — the forecast processor extracts the `amount` value.

### Report 6 (Monthly Quota) Special Handling

Report 6 uses `_label_User` for rep names (the `User` field contains raw Salesforce IDs). Currency fields (`Funded Dollars`, `Funded Dollars Quota`, etc.) come as `{amount: float, currency: null}` dicts. The forecast processor in `processors/forecast.py` handles both Salesforce live data and static fallback from `forecast_data.py`.

### Maps Check-In Split Fetch

Report 5 (Maps Check-Ins) is fetched in two API calls to avoid Salesforce's 2,000 row limit:
- Call 1: Created Date from 1st of month through mid-month
- Call 2: Created Date from mid-month through today
Results are merged with deduplication by the pipeline. The split point is calculated automatically.
...
1. **Script data block replacement**: Replaces content between `<script>` tag and first `function` keyword. Used for JS data variables.
2. **Marker-based replacement**: Replaces content between `<!-- Marker Name -->` and `<!-- /Marker Name -->` comment pairs. Used for:
