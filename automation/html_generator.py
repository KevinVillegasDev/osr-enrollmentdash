...
def _generate_forecast_table(forecast_data: dict) -> str:
    ...
    table = (
        ...
        f'<thead><tr>'
        f'<th class="fc-th-name" style="cursor:help" title="Outside sales rep and their assigned territory code">Rep</th>'
        f'<th class="fc-th" style="cursor:help" title="Total funded origination dollars in this territory so far this month — includes all merchants (new enrollees + existing book of business)">MTD Actual</th>'
        f'<th class="fc-th" style="cursor:help" title="Monthly budget target for this territory from the 2026 sales budget — based on total territory production, not just new enrollments">Budget</th>'
        f'<th class="fc-th" style="cursor:help" title="Projected end-of-month total based on current daily pace — calculated as: MTD Actual × (total business days ÷ elapsed business days)">Projected</th>'
        f'<th class="fc-th" style="cursor:help" title="Percentage difference between projected end-of-month and budget target — green = on/above target, amber = within 10%, red = more than 10% behind">Variance</th>'
        f'<th class="fc-th" style="width:140px;cursor:help" title="Visual progress bar showing projected production as a percentage of the budget target — full bar means on track to meet or exceed budget">Pace</th>'
        f'</tr></thead>\n'
        f'<tbody>\n'
        + "\n".join(rows) +
        f'\n</tbody>\n'
